#!/bin/bash
# ChromeOSFetch test build - clean solid-block Chrome logo
...
# 34-column Chrome-style block logo. No slash or backslash characters are used.
printf '           %s████████████%s           %s%s%s\n' "$RED" "$R" "$W" "$user@$host" "$R"
printf '      %s██████████████████████%s      %s------------------------%s\n' "$RED" "$R" "$W" "$R"
printf '   %s████████████████████%s████████%s   %sOS:%s %s %s (%s)\n' "$RED" "$Y" "$R" "$B" "$R" "$os_name" "$os_version" "$arch"
printf ' %s██████████████████%s██████████████%s %sHost:%s %s\n' "$RED" "$Y" "$R" "$B" "$R" "$hwid"
printf '%s████████████%s   %s████%s   %s████████████%s  %sKernel:%s %s\n' "$RED" "$R" "$B" "$R" "$Y" "$R" "$G" "$R" "$kernel"
printf '%s██████████%s  %s██████████%s  %s██████████%s  %sUptime:%s %s\n' "$G" "$R" "$B" "$R" "$Y" "$R" "$G" "$R" "$uptime"
printf '%s█████████%s  %s████████████%s  %s█████████%s  %sShell:%s %s\n' "$G" "$R" "$B" "$R" "$Y" "$R" "$Y" "$R" "$shell_name"
printf '%s██████████%s  %s██████████%s  %s██████████%s  %sCPU:%s %s\n' "$G" "$R" "$B" "$R" "$Y" "$R" "$Y" "$R" "$cpu"
printf '%s████████████%s   %s████%s   %s████████████%s  %sGPU:%s %s\n' "$G" "$R" "$B" "$R" "$Y" "$R" "$C" "$R" "$gpu"
printf ' %s██████████████████████%s██████████%s %sMemory:%s %s\n' "$G" "$Y" "$R" "$C" "$R" "$memory"
printf '   %s████████████████████████%s████%s   %sDisplay:%s %s\n' "$G" "$Y" "$R" "$B" "$R" "$displays"
printf '      %s██████████████████████%s      %sBattery:%s %s\n' "$G" "$R" "$B" "$R" "$battery"
printf '           %s████████████%s           %sDisk:%s %s\n' "$G" "$R" "$G" "$R" "$disk"
