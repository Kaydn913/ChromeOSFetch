...
printf "${C}      _________              ${W}%s@%s${R}\n" "$user" "$host"
printf "${C}     /         \\\\             ${W}------------------------${R}\n"
printf "${B}    |  Chrome   |            OS${R}: %s %s (%s)\n" "$os_name" "$os_version" "$arch"
printf "${B}    |    OS     |          Host${R}: %s\n" "$hwid"
printf "${G}     \\\\_________/         Kernel${R}: %s\n" "$kernel"
printf "${G}       _| |__           Uptime${R}: %s\n" "$uptime"
printf "${Y}      /______/            Shell${R}: %s\n" "$shell_name"
printf "${Y}                          CPU${R}: %s\n" "$cpu"
printf "${C}                          GPU${R}: %s\n" "$gpu"
printf "${C}                       Memory${R}: %s\n" "$memory"
printf "${B}                      Display${R}: %s\n" "$displays"
printf "${B}                      Battery${R}: %s\n" "$battery"
printf "${G}                         Disk${R}: %s\n" "$disk"
printf "${R}\n"
